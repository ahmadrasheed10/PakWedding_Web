...
function CustomDropdown({
  ...
  const displayIcon = selectedOption?.icon || (iconMap && typeof value === 'string' && iconMap[value]) || icon
  ...
  return (
    <div ref={dropdownRef} className="relative w-full">
      <button
        ...
        className={`w-full h-14 px-5 rounded-xl border-2 transition-all duration-200 flex items-center justify-between bg-white text-left font-medium text-sm shadow-sm cursor-pointer ${
          isOpen
            ? 'border-[#D72626] ring-2 ring-[#D72626]/20'
            : value !== '' && value !== 'popular' && value !== 0
            ? 'border-[#D72626]/70 text-gray-900 bg-red-50/20'
            : 'border-gray-200 text-gray-700 hover:border-gray-300'
        }`}
      >
        <div className="flex items-center gap-2 truncate pr-2">
          {displayIcon && <span className="text-base flex-shrink-0">{displayIcon}</span>}
          <span className="truncate">{displayLabel}</span>
        </div>
        <svg
          className={`w-5 h-5 text-gray-400 transition-transform duration-200 flex-shrink-0 ${
            isOpen ? 'rotate-180 text-[#D72626]' : ''
          }`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 z-50 bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-64 overflow-y-auto py-2">
          {options.map((opt) => {
            const isSelected = String(opt.value) === String(value)
            const optIcon = opt.icon || (iconMap && typeof opt.value === 'string' && iconMap[opt.value])
            return (
              <button
                ...
                className={`w-full px-4 py-2.5 text-left text-sm flex items-center justify-between transition-colors cursor-pointer ${
                  isSelected
                    ? 'bg-gradient-to-r from-[#D72626]/10 to-[#F26D46]/10 text-[#D72626] font-bold'
                    : 'text-gray-700 hover:bg-gray-50'
                }`}
              >
                <div className="flex items-center gap-2.5 truncate pr-2">
                  {optIcon && <span className="text-base flex-shrink-0">{optIcon}</span>}
                  <span className="truncate">{opt.label}</span>
                </div>
                {isSelected && (
                  <svg className="w-4 h-4 text-[#D72626] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className={`flex-1 flex flex-col overflow-y-auto ${isAuthed ? 'pt-16 lg:pt-0' : ''}`}>
        ...
        <div className="container mx-auto max-w-6xl px-6 sm:px-8 py-10 sm:py-12 relative">
          ...
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              type="submit"
              className="px-12 h-14 rounded-xl text-white font-semibold bg-gradient-to-r from-[#D72626] via-[#F26D46] to-[#F7A76C] hover:opacity-90 transition-all shadow-lg hover:shadow-xl cursor-pointer"
            >
              Search
            </button>
          </form>
          ...
          <div className="mb-6 overflow-x-auto pb-2">
            <div className="flex items-center gap-2.5 min-w-max">
              <button
                ...
                className={`px-4 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 shadow-sm flex items-center gap-2 border cursor-pointer ${
                  selectedCategory === ''
                    ? 'bg-gradient-to-r from-[#D72626] to-[#F26D46] text-white border-transparent shadow-md scale-105'
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <CustomDropdown
              ...
              options={[
                ...
                { label: '4.5+ Stars', value: 4.5, icon: '🌟' },
                { label: '5 Stars', value: 5, icon: '✨' },
              ]}
            />
              ...
              {filteredVendors.length > vendorsPerPage && (
                <div className="flex justify-center items-center gap-3 mt-8">
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg border border-gray-200 transition-all ${currentPage === 1
                      ...
                      return (
                        <div key={page} className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className={`w-10 h-10 rounded-lg transition-all ${page === currentPage
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg border border-gray-200 transition-all ${currentPage >= Math.ceil(filteredVendors.length / vendorsPerPage)
                        ? 'text-gray-300 cursor-not-allowed'
                        : 'text-gray-500 hover:bg-gray-50 hover:border-pink-300'
                      }`}
                  >
